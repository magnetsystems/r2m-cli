...
class ClearCommand extends AbstractCommand {
   ...
   */
  ClearCommand(String name, List<String> aliases, boolean hidden) {
  ...
  def execute(List<String> args) {
    if (getShell().isConsole()) {
      String prompt = getShell().getReader().getDefaultPrompt()
      getShell().getReader().setDefaultPrompt('')
      getShell().getReader()?.clearScreen()
      getShell().getReader().setDefaultPrompt(prompt)
    }
  }
