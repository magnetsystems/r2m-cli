...
public class ConsoleReader implements ConsoleOperations {
  ...
  private int autoprintThreshhold = Integer.getInteger(
      "jline.completion.threshold", 100); // same default as
  ...
  int getCursorPosition() {
    ...
    // absolute position
    return getStrippedAnsiLength(prompt) + UnicodeUtils.getPrintLength(buf.getBuffer().subSequence(0, buf.cursor));
  }
  ...
  int getStrippedAnsiLength(String ansiString) {
    ...
    for (int i = 0; i < chars.length; i++) {
      ...
      if (!inAnsi && c == 27 && i < chars.length - 1 && chars[i + 1] == '[') {
        ...
        int inc = UnicodeUtils.getPrintLength(Character.toString(c));
        strippedLength = strippedLength + inc;
      }
    }
      ...
      while (true) {
        ...
        if (state == NORMAL) {
          switch (code) {
            case EXIT: // ctrl-d
              printNewline();
              throw new JlineInterruptException();
            case COMPLETE: // tab
              success = complete();
              break;
  ...
  private boolean complete() throws IOException {
    ...
    for (Object completor : completors) {
      Completor comp = (Completor) completor;
   ...
   * @param num the number of character to go back
   */
